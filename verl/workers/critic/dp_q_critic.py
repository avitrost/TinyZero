...
__all__ = ['DataParallelPPOQCritic']

def extract_values(values, sampled_index):  # Option 1 # TODO: check correctness
...
class DataParallelPPOQCritic(BasePPOCritic):
    ...
    def _forward_micro_batch(self, micro_batch, sampled_index):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                values = values[:, -response_length - 1:-1]
                sampled_index = input_ids[:, -response_length:]  # index of the next token in the sequence
                values = extract_values(values, sampled_index)
            return values  # returns unnormalized logits over vocabulary
